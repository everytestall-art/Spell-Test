...
# =========================================================
# JSON CORRECT-WORD DICTIONARY (WHITELIST)
# =========================================================

# รายการคำในไฟล์นี้ถือเป็น "คำถูก" และมีสิทธิ์เหนือผลตรวจจาก Engine เดิม
JSON_DICTIONARY_PATH = Path(
    r"C:\Users\ADMIN\OneDrive\Desktop\SpellCheck\dictionary.json"
)


def load_json_correct_words(json_path: Path) -> tuple:
    """
    โหลดคำถูกจาก dictionary.json โดยไม่ทำให้โปรแกรมหยุดทำงานเมื่อไฟล์มีปัญหา

    รูปแบบ JSON ที่รองรับ เช่น
    1) ["คำที่หนึ่ง", "คำที่สอง"]
    2) {"words": ["คำที่หนึ่ง", "คำที่สอง"]}
    3) {"correct_words": ["คำที่หนึ่ง", "คำที่สอง"]}
    4) {"dictionary": ["คำที่หนึ่ง", "คำที่สอง"]}
    5) {"คำที่หนึ่ง": true, "คำที่สอง": true}

    คืนค่า: (set_of_words, status_message, loaded_ok)
    """
    ...
    try:
        ...
        try:
            ...
            # รองรับ trailing comma เช่น ["คำ1", "คำ2",]
        ...
        return (
            set(),
            f"ไม่พบไฟล์คลังคำ: {json_path}",
            False,
        )
    except json.JSONDecodeError as exc:
        return (
            set(),
            f"ไฟล์ dictionary.json มีรูปแบบ JSON ไม่ถูกต้อง: {exc}",
            False,
        )
    except Exception as exc:
        return (
            set(),
            f"ไม่สามารถอ่าน dictionary.json ได้: {exc}",
            False,
        )
    ...
    if isinstance(data, list):
        ...
        # รองรับ object ที่เก็บรายการคำไว้ใน key ที่พบบ่อย
        for key in ("words", "correct_words", "dictionary", "correct", "whitelist"):
            ...
            # หากเป็น {"คำ": true, ...} หรือ {"คำ": {...}, ...}
            raw_words = list(data.keys())

    else:
        return (
            set(),
            "dictionary.json ต้องเป็น JSON Array หรือ JSON Object",
            False,
        )
    ...
    return (
        correct_words,
        f"โหลดคลังคำถูกจาก JSON แล้ว {len(correct_words):,} คำ",
        True,
    )


JSON_CORRECT_WORDS, JSON_DICTIONARY_STATUS, JSON_DICTIONARY_OK = (
    load_json_correct_words(JSON_DICTIONARY_PATH)
)
...
def detect_thai_ranked_errors(text: str) -> tuple:
    ...
    for width in (3, 2):
        for i in range(0, len(tokens) - width + 1):
            ...
            #
            # เดิม structure_score >= 0.60 เข้มเกินไปสำหรับกรณี
            # เช่น "พวกหรีด" -> "พวงหรีด"
            #
            # ทั้งสองคำต่างกันเพียง 1 ตัวอักษรและ similarity สูง
            # แต่ common-prefix/suffix ทำให้ structure_score อยู่ราว 0.56
            # จึงหลุดจาก Engine แม้ candidate จะใกล้มาก
            #
            # ปรับให้ใช้หลายหลักฐานร่วมกันแทนการตัดด้วย
            # structure_score ค่าเดียว เพื่อยังควบคุม false positive
            # -------------------------------------------------
            structure_score = best.get("structure_score", 0)
            context_score = best.get("context_score", 0)
            ...
            structural_support = (
                structure_score >= 0.53
            )

            contextual_support = (
                context_score >= 0.18
                and candidate_margin >= 0.015
            )
            ...
            errors.append({
                ...
                "structure_score": best.get("structure_score", 0),
                "detail": (
                    "ตรวจพบว่าคำอาจถูกตัวแบ่งคำแยกออกเป็นหลายส่วน "
                    "จึงรวม Token แล้วเปรียบเทียบ Candidate จาก "
                    "Edit Distance, ความคล้ายของรูปคำ, โครงสร้างคำ "
...
# =========================================================
# Highlight เฉพาะ "ตัวเลขจำนวนที่ตรวจสอบ"
# - ค่าที่เลือกอยู่ตอน Dropdown ปิด
# - ทุกตัวเลือกตอนเปิด Dropdown
# รูปแบบ: หน้า 1 · ผิด [6] คำ · ไม่แน่ใจ [1] คำ
components.html(
    fr"""
    <script>
    (function () {{
        ...
        const OVERLAY_CLASS = "spell-count-selected-overlay";

        const pattern =
            /(ผิด\s*)(\d+)(\s*คำ\s*·\s*ไม่แน่ใจ\s*)(\d+)(\s*คำ)/;

        function createCount(value) {{
            const mark = parentDoc.createElement("span");
            mark.className = HIGHLIGHT_CLASS;
            mark.textContent = value;

            // Highlight สีเหลืองตามตัวอย่าง
            mark.style.backgroundColor = "#FFF200";
            mark.style.color = "#111111";
            mark.style.fontWeight = "900";
            mark.style.fontSize = "1.08em";
            mark.style.padding = "1px 4px";
            mark.style.margin = "0 2px";
            mark.style.borderRadius = "2px";
            mark.style.lineHeight = "1.35";
            mark.style.display = "inline-block";
            mark.style.boxShadow = "inset 0 -1px 0 rgba(0,0,0,.08)";
            return mark;
        }}

        function renderLabel(container, label) {{
            ...
            container.appendChild(createCount(match[2]));
            container.appendChild(
                parentDoc.createTextNode(match[3])
            );
            container.appendChild(createCount(match[4]));
            container.appendChild(
        ...
        function getPageSelect() {{
            ...
            // เลือก Selectbox ที่มีข้อความรูปแบบ
            // "ผิด N คำ · ไม่แน่ใจ N คำ"
            for (const box of boxes) {{
                ...
                if (
                    ...
                    pattern.test(text) ||
                    (SELECTED_LABEL && text.includes("เลือกหน้าเอกสาร"))
                ) {{
                    return box;
                }}
            }}

            // หน้าเอกสารเป็น Selectbox หลักของหน้านี้
            return boxes.length ? boxes[0] : null;
        }}

        function decorateSelectedValue() {{
            const box = getPageSelect();
            if (!box || !SELECTED_LABEL || !pattern.test(SELECTED_LABEL)) {{
                return;
            }}
            ...
            let overlay = control.querySelector("." + OVERLAY_CLASS);
            ...
            if (!overlay) {{
                ...
                const computed =
                    window.parent.getComputedStyle(control);
                ...
                // พื้นหลัง overlay ทับข้อความ native
                // โดยไม่ทำให้ Dropdown กดไม่ได้
            ...
            if (overlay.dataset.label !== SELECTED_LABEL) {{
                overlay.dataset.label = SELECTED_LABEL;
                renderLabel(overlay, SELECTED_LABEL);
            }}
        }}
        ...
        function decorateOptions() {{
            parentDoc
                ...
                .forEach(function (option) {{
                    if (option.dataset.countHighlighted === "1") {{
                        return;
                    }}
                    ...
                    // เปลี่ยนเฉพาะเนื้อหาของ option
                    // ไม่แก้ event / role ของ Dropdown
                    if (renderLabel(option, label)) {{
                        option.dataset.countHighlighted = "1";
                    }}
                }});
        }}
        ...
        function scan() {{
            ...
            window.requestAnimationFrame(function () {{
                ...
                try {{
                    decorateSelectedValue();
                    decorateOptions();
                }} catch (err) {{
                    console.warn(
                        "ไม่สามารถ Highlight จำนวนใน Dropdown ได้",
                        err
                    );
                }}
            }});
        ...
        // รองรับจังหวะ render ของ Streamlit / BaseWeb
        [50, 120, 250, 500, 900, 1500].forEach(function (delay) {{
